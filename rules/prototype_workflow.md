...
> V1.6 读取规则：先读 `prototype/_index.md`，需要生成或修复某页时才读取具体 HTML。
...
2. **创建文件夹**：从需求文档文件名或文档内容中提取版本号，创建 `prototype/{版本号}/`，写入状态：`active_workflow=prototype, phase=3, prototype_folder=prototype/{版本号}, prototype_version={版本号}, last_checkpoint=phase3_generating, current_node=prototype.generate_pages, process_file=memory/process/prototype.generate_pages.md`
   - 不同版本原型不得混放。
   - 若无法识别版本号：
...
5. 更新状态：`last_checkpoint=phase3_review_in_progress, current_node=prototype.review`
6. 用 Read 工具读取 `rules/review_prototype.md`，替换 `{需求文档全文}` 和 `{原型清单及内容}` 后，通过 Task 工具发送给 Sub-Agent
7. 存储结果至 `last_prototype_review_result`，更新 `last_checkpoint=phase3_review_done, current_node=prototype.review`。有问题则写入 `memory/project_memory.md` 的待确认问题并修复（多页面问题可再次并行）后重新 review，等用户确认定稿。
8. 用户确认原型定稿后写入：`prototype_finalized=true, last_checkpoint=prototype_to_demo_choice, current_node=prototype.to_demo_choice`，更新 `memory/project_memory.md` 清理已解决的待确认问题。
...
□ 是否更新了当前 `process_file`、`memory/handoff.md` 和 `prototype/_index.md`？
